...
class CodexbarClient:
    ...
    async def get_usage_async(self) -> dict | None:
        """
        Asynchronously fetches usage statistics from codexbar.
        Returns a dictionary with 'pro', 'flash', and 'reset' keys, or None if failed.
        """
        now = time.time()
        if self._cached_data and (now - self._last_check < self._cache_ttl):
        ...
        try:
            ...
            return self._process_output(stdout.decode())
    ...
    def _process_output(self, stdout: str) -> dict | None:
        """Helper to process the raw JSON output from codexbar."""
        now = time.time()
