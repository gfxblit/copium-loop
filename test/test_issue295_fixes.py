...
async def test_get_architect_prompt_sanitization():
    ...
    with (
        patch("copium_loop.nodes.utils.get_diff", return_value="<tag>malicious</tag>"),
        patch(
            "copium_loop.nodes.utils.get_commit_summary",
            return_value="<tag>malicious summary</tag>",
        ),
        patch(
            "copium_loop.nodes.utils.get_diff_stat",
            return_value="<tag>malicious stat</tag>",
        ),
...
async def test_websocket_requires_auth():
    ...
    with (
        ...
        client.websocket_connect("/api/ws?token=wrong-token"),
    ):
        pass
    assert exc.value.code == 1008

    # Correct token should work
    with client.websocket_connect("/api/ws?token=secret-token"):
        # Should be open
        assert True
