...
class TestConditionalsReproduction:
    ...
    def test_all_nodes_ignore_stale_errors(self):
        ...
        # Coder
        assert should_continue_from_coder({"code_status": "coded", "last_error": stale_error}) == "tester"
        # Architect
        assert should_continue_from_architect({"architect_status": "ok", "last_error": stale_error}) == "reviewer"
        # Reviewer
        assert should_continue_from_review({"review_status": "approved", "last_error": stale_error}) == "pr_pre_checker"
        # PR Pre-Checker
        assert should_continue_from_pr_pre_checker({"review_status": "pre_check_passed", "last_error": stale_error}) == "pr_creator"
        # PR Creator
        assert should_continue_from_pr_creator({"review_status": "pr_created", "last_error": stale_error}) == END
        # Journaler
        assert should_continue_from_journaler({"review_status": "pending", "last_error": stale_error}) == END
