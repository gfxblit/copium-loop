...
class TestTesterNode:
    ...
    async def test_tester_returns_pass(self):
        ...
        with patch('copium_loop.nodes.tester.run_command', new_callable=AsyncMock) as mock_run:
    ...
    async def test_tester_returns_fail(self):
        ...
        with patch('copium_loop.nodes.tester.run_command', new_callable=AsyncMock) as mock_run:
            ...
            with patch('copium_loop.nodes.tester.notify', new_callable=AsyncMock) as mock_notify:
                state = {'retry_count': 0}
                result = await tester(state)
